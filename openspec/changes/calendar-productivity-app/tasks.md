...
- [ ] 5.16 Redesign the onboarding tutorial (`OnboardingContext.tsx`/`OnboardingTutorial.tsx`) into a spotlight-style, per-feature guide instead of one plain scrolling list of text sections. Two levels: a short global tour ("how the app works", current content trimmed/kept) and a per-feature tour for each major area (calendar & schedules, tasks & quick-add, pomodoro, block list — hard mode & triggers, serotonin mode & rituals, habits, detox), each launchable on its own. Each tour step visually spotlights the actual on-screen card/panel it's talking about and dims everything else, instead of a floating card with no connection to the page underneath. Reuses the `data-tutorial` attributes already placed through the app for exactly this purpose but currently unused for any highlighting (`app-nav` in `AppNav.tsx`; `calendar-hero`, `schedule-header`, `tasks-header`, `serotonin-mode` via `PageHeader`'s `tutorialId` prop and inline in `CalendarPage.tsx`; `day-work-block`/`day-rest-block` in `DayView.tsx`) — extend the set to cover panels added since (block list & triggers, habits, quick-add). Add a persistent "?" affordance next to the existing "Tutorial" button in `AppNav.tsx` that opens a menu to launch the global tour or jump straight to any feature's tour, available any time, not just on first login. Update tutorial copy to cover everything shipped since the current tutorial was written: the habit tracker, the "Q" quick-add shortcut, block-list hard mode (5.6), the breathing/friction pause (5.7), location/Wi-Fi focus triggers (5.8), and temptations-avoided scoring (5.9). **Pendiente de ampliar cuando se implementen 5.14/5.15**: el tour de Pomodoro/foco debe cubrir el widget de sonido ambiental (5.14) y el tour de lista de bloqueo debe cubrir la fricción por ejercicio verificada por cámara (5.15) — ninguna de las dos existe todavía, así que no se agregan al catálogo hasta que se construyan.
