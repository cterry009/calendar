...
- [x] 5.12 Decide whether to add a light theme / `prefers-color-scheme` support alongside the existing dark/calm themes, or intentionally commit to dark-only as part of the focus/dopamine-reduction branding. Decided: dark-only. Verified there's no partial light-mode path to clean up first: `CalendarTheme` (`packages/ui/src/tokens.ts`) only has `dark`/`calm` keys, `CalendarProvider` defaults to and only ever renders `theme="dark"` (`apps/web/src/main.tsx` never overrides it), and there is zero `prefers-color-scheme`/`matchMedia`/color-scheme code anywhere in the repo -- confirmed by grep and by reading `apps/web/index.html`. `packages/ui/src/tamagui.config.ts` does spread Tamagui's full `defaultConfig.themes` (which includes an unstyled, unused `light` theme from the library default) before overriding `dark`/`calm` -- confirmed inert (nothing in the app ever requests it, and the `CalendarTheme` type doesn't allow passing `"light"` to `CalendarProvider`), left alone rather than risk a theme-registry refactor for a purely cosmetic cleanup with no functional benefit.
