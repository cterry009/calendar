...
**Qué se verificó y qué no (primera pasada, solo web)**: typecheck y lint limpios. Vía `expo start --web` + Playwright: la pantalla monta sin errores, el fallback "no disponible en web" se muestra en vez del picker (prueba que el guard de `Platform.OS` funciona y que el módulo nativo no rompe el bundle web), y el listado/borrado de entradas existentes -- que no toca el módulo nativo en absoluto -- funciona de punta a punta contra el servidor real.

**Actualización -- el usuario pidió instalar el toolchain, y se compiló de verdad**: se instaló JDK 17 (Temurin, vía `winget`, sin instalador interactivo) y el SDK de Android completo por línea de comandos (`cmdline-tools` descargado directo de Google, `platform-tools`, `platforms;android-35`, `build-tools;35.0.0`, licencias aceptadas con `yes |`), deliberadamente sin el instalador gráfico de Android Studio -- su wizard de primer arranque es interactivo y no se puede scriptear, y las SDK command-line tools logran exactamente lo mismo para compilar. Con `ANDROID_HOME`/`JAVA_HOME` configurados, `npx expo prebuild -p android` + `./gradlew assembleDebug` corrieron de punta a punta: **`BUILD SUCCESSFUL`**, con un `app-debug.apk` real. Se confirmó con `aapt dump badging` y una inspección de los `.dex` de la APK que el permiso `QUERY_ALL_PACKAGES` está en el manifest fusionado final y que el bytecode de `react-native-launcher-kit` está efectivamente linkeado (no solo declarado en `package.json`).

**Bug real atrapado por este build, que ninguna verificación web podría haber encontrado**: `AndroidConfig.Permissions.ensurePermissions()` de `@expo/config-plugins` muta el manifest en el lugar y devuelve un mapa de estado `{ [permiso]: boolean }` -- no el manifest modificado. La primera versión del plugin hacía `config.modResults = AndroidConfig.Permissions.ensurePermissions(...)`, reemplazando el manifest real completo por ese mapa de estado, que el serializador de config-plugins convirtió en un XML corrupto de una sola línea (`<android.permission.QUERY_ALL_PACKAGES>true</...>`, sin `<application>` ni actividades -- la app no habría arrancado). El primer `gradlew assembleDebug` igual dio `BUILD SUCCESSFUL` porque Gradle no valida la estructura semántica del manifest de origen antes de fusionarlo con los manifests de las librerías, así que este bug solo se detectó inspeccionando el manifest fusionado a mano, no por el resultado del build en sí. Corregido dejando que la mutación en el lugar haga su trabajo, sin reasignar el valor de retorno. Esto confirma por qué instalar el entorno real valió la pena más allá de "ver si compila": encontró un bug que ninguna de las verificaciones anteriores (typecheck, lint, Playwright en web) podía haber atrapado, porque ninguna toca el manifest de Android.

**De paso**: se fijó `android.package` en `app.json` (Expo le había puesto el placeholder `com.anonymous.calendarproductivitymobile` automáticamente al no haber ninguno) a `app.calendarproductivity.mobile`, decisión explícita del usuario -- este id es costoso de cambiar después de publicar en Play Store, mejor decidirlo ahora que es gratis.

**Lo que sigue sin poder verificarse acá**: correr la app de verdad. Compilar y linkear no requiere emulador ni dispositivo, pero ejecutarla sí -- necesita un emulador (requiere soporte de virtualización, no confirmado en este entorno) o un teléfono físico conectado por `adb` (no disponible acá). Por lo tanto, si `getSortedApps()` devuelve datos reales de apps instaladas en un dispositivo real sigue sin confirmarse -- es el único paso que falta.

### 35. Accessibility Service para bloqueo real de apps — tareas 6.6/6.7/6.8

**Decisión de arquitectura -- módulo local de Expo, no una librería de terceros**: a diferencia de 6.5 (donde `react-native-launcher-kit` ya resolvía el problema), acá no hay ningún paquete de terceros para "observar la app en primer plano y actuar" según la propia lista de bloqueo/pomodoro/horarios de esta app -- es lógica genuinamente propia. Se creó `apps/mobile/modules/focus-block/` como módulo local de Expo (`npx create-expo-module --local`, manteniendo solo Android+web -- iOS no tiene un equivalente de Accessibility Service para terceros y no está en el alcance de esta fase), en vez de eyectar del workflow gestionado (CNG) o escribir el módulo a mano fuera de la convención de Expo.

**Puente entre JS y el servicio nativo**: un `AccessibilityService` corre independiente del motor de JS -- no puede simplemente "preguntarle a React" si debe bloquear en cada cambio de app en primer plano. La única vía elegida: `FocusBlockPrefs.kt`, un wrapper de `SharedPreferences` (mismo proceso, síncrono, barato de leer en cada evento) que `FocusBlockModule.kt` escribe (desde `apps/mobile/src/hooks/useFocusBlocking.ts`, cada vez que cambia el estado combinado) y que `FocusBlockAccessibilityService.kt` lee en cada `onAccessibilityEvent`.

**"Modo tarea" no es un disparador aparte**: se confirmó contra el código real de `apps/web/src/context/SoftFocusContext.tsx` (no por suposición) que ahí tampoco lo es -- una sesión de pomodoro con `taskId` pasa por el mismo flag `isBlocking` que cualquier otra. `useFocusBlocking.ts` combina exactamente dos condiciones independientes, igual que la web: `isPomodoroBlocking` (ya existía) e `isWorkHoursActive` (portado recién, `isNowWithinWorkSchedule`/`findActiveWorkSchedule`, no existía en absoluto en móvil hasta ahora). Los disparadores `FocusTrigger` (LOCATION/WIFI) quedan deliberadamente sin portar -- WIFI ni siquiera se aplica en la web (sin API de SSID en el navegador), y LOCATION requeriría `expo-location` + una lógica de geofencing completa, un recorte real para otra pasada, no un vacío silencioso.

**Bugs reales atrapados por el build real, ninguno detectable por verificación web**: (1) el fragmento de manifest del módulo no declaraba `xmlns:android`, causando `ManifestMerger2$MergeFailureException` (prefijo `android:` sin enlazar); (2) `FocusBlockModule.kt` tenía un caso límite real de inferencia de tipos de Kotlin -- un `return@Function` sin valor dentro de una expresión elvis, seguido de una última sentencia que devuelve `Unit`, producía "Return type mismatch: expected 'Any?', actual 'Unit'" desde el DSL de Expo Modules Kotlin. Arreglado reescribiendo las tres funciones del puente con `appContext.reactContext?.let { }` en vez de retorno anticipado. Ambos bugs se encontraron porque el toolchain de Android ya estaba instalado (ver actualización de la decisión 34) y se corrió `gradlew assembleDebug` de verdad -- ninguno de los dos podría haberse atrapado con `expo start --web` + Playwright, porque ninguno de los dos toca código que el flujo web ejecuta.

**Verificación, la misma frontera de siempre**: `BUILD SUCCESSFUL` real, con el servicio confirmado en el manifest fusionado y su bytecode confirmado dentro del APK (mismo nivel de prueba que 6.5). Lo que la compilación *no* puede probar -- si el evento de accesibilidad realmente dispara al cambiar de app, si `performGlobalAction(GLOBAL_ACTION_HOME)` se comporta como se espera, si el deep link reabre la app en un dispositivo real -- sigue sin poder verificarse en este entorno (sin emulador ni dispositivo). Lo que sí se pudo verificar con `expo start --web` + Playwright, y tiene valor real aunque sea parcial: la lógica de decisión (`useFocusBlocking.ts`) es TypeScript puro e independiente de plataforma, así que se puede observar corriendo igual en cualquier lado -- se confirmaron los tres escenarios de la tarea 6.8 (estado base, pomodoro activo, horario de trabajo activo sin pomodoro corriendo) con los valores exactos esperados, más la pantalla `/blocked` resolviendo correctamente la etiqueta de una entrada real de la lista de bloqueo.

### 36. Verificación en dispositivo real — tarea 6.9

**Contexto**: con el toolchain de Android ya instalado (decisión 34) y un teléfono físico (Samsung Galaxy S23, Android 16) conectado por USB, se pudo por fin correr código real en Android en vez de solo compilarlo o simularlo en la vista previa web. `adb reverse` sobre los puertos 3000 (servidor) y 8081 (Metro) le permitió al teléfono alcanzar el backend y el dev client igual que si corrieran localmente.

**Confirmado funcionando de punta a punta**: la enumeración real de apps instaladas (6.5, con nombres de paquete reales del dispositivo), el `AccessibilityService` uniéndose de verdad al sistema (confirmado con `dumpsys accessibility`, no solo con la cadena cruda de ajustes), y la intercepción real -- abrir una app bloqueada durante un pomodoro activo la manda de verdad a la pantalla de inicio (`performGlobalAction(GLOBAL_ACTION_HOME)` confirmado con `dumpsys window` cambiando de foco en el mismo evento).

**Bug real de Android encontrado (no de esta app)**: hacer `force-stop` de la app desde `adb` desvincula el `AccessibilityService` en silencio -- la cadena cruda de `Settings.Secure.enabled_accessibility_services` sigue listando el servicio y el toggle en Ajustes sigue mostrando "On", pero `dumpsys accessibility` deja de listarlo en `Bound services`, y no se re-vincula solo con reabrir la app -- hace falta volver a Ajustes de accesibilidad y tocar el toggle de nuevo. Esto es relevante más allá de esta sesión de pruebas: cualquier mecanismo real de Android que fuerce el cierre de la app (gestión de batería, "apps sin usar", etc.) dejaría el bloqueo silenciosamente inactivo hasta que el usuario vuelva a Ajustes -- vale la pena, en una pasada futura, que la propia app detecte esto (comparando `isAccessibilityServiceEnabled()` contra el estado esperado) y avise en vez de fallar en silencio.

**Bug real encontrado en 6.7, solo detectable en hardware real**: el `startActivity()` hacia la pantalla `/blocked`, disparado desde el `AccessibilityService` justo después de mandar al usuario a inicio, nunca llega a mostrarse -- confirmado sondeando `dumpsys window` cada 0.5s durante 2.5s tras el evento, el launcher se queda enfocado todo el tiempo. Esto coincide exactamente con las restricciones de Android 10+ sobre iniciar actividades en segundo plano (BAL): un `Service` reaccionando a un evento del sistema (no una interacción directa y actualmente en primer plano del usuario) no tiene permiso por defecto para abrir una `Activity` nueva. Ninguna verificación posible sin dispositivo real (ni la compilación, ni la inspección del manifest/bytecode, ni Playwright en la vista previa web) podía haber revelado esto -- la pantalla `/blocked` en sí renderiza perfectamente bien cuando se llega a ella directamente, el problema es específicamente llegar ahí automáticamente desde el servicio. Dos soluciones reales y estándar quedan documentadas como trabajo de seguimiento (no implementadas a ciegas sin poder iterar con más pruebas en dispositivo): (a) permiso `SYSTEM_ALERT_WINDOW` + una vista superpuesta nativa dibujada directamente desde el servicio -- el patrón que ya usan las otras apps de bloqueo instaladas en este mismo teléfono de prueba; (b) una `Notification` de alta prioridad con `fullScreenIntent`, que Android sí permite usar para abrir una Activity desde segundo plano.
