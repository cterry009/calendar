import { calculateEstimationAccuracy, calculateFitnessProductivityCorrelation, type TaskDifficulty } from '@calendar/shared';
import type { DashboardMetrics, DashboardSnapshot, DashboardWeekMetric } from './types';

// Ported from apps/web/src/lib/analytics/aggregate.ts -- pure TS (only Date/toLocaleDateString),
// no DOM dependency, reused verbatim. `suggestions` is dropped: apps/web's own DashboardPage.tsx
// never renders them either (that's a separate suggestions panel elsewhere), so porting
// generateSuggestions()/buildSuggestionInput() here would be dead weight.
...
export function buildDashboardMetrics(snapshot: DashboardSnapshot, now = new Date()): DashboardMetrics {
  ...
  const completedPomodoros = snapshot.pomodoroSessions.filter((session) => !session.interrupted && !!session.endedAt);
