...
import {
  buildCreatePomodoroPayload,
  buildUpdatePomodoroPayload,
  syncPomodoroBatch,
} from '../lib/pomodoro/sync';
...
function withRecordMetadata(
  base: Parameters<typeof createPomodoroSession>[0] extends never ? never : ReturnType<typeof createPomodoroSession>,
...
export function usePomodoro(): UsePomodoroResult {
  ...
  useEffect(() => {
    ...
    if (remainingSeconds > 0 || transitionInFlightRef.current) {
      return;
    }
