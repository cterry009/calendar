...
async function refreshAccessToken(): Promise<string | null> {
  ...
  if (!response.ok) {
    ...
    // connection alone could force a full re-login. Just fail this one request; the stored tokens
    // are untouched, so the next successful request can refresh normally.
    return null;
  }
